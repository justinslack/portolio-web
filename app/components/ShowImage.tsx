...
interface ShowImageProps extends Omit<ImageProps, 'src' | 'onError'> {
  src: string;
  alt: string;
}

export function ShowImage({ src, alt, className, ...props }: ShowImageProps) {
  ...
  if (!hasValidImagePath || imageError) {
    return (
      <div className={`flex items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200 ${className || ''}`}>
        <Radio className="h-24 w-24 text-gray-400" />
      </div>
    );
  }
